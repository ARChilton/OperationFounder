L.TileLayer.addInitHook(function () {
	...
	// ARC updated the db connection from this._db to offlineTiles
	offlineTiles = new PouchDB('offline-tiles');
...
L.TileLayer.prototype.options.cacheMaxAge = 30 * 24 * 3600 * 1000;
...
L.TileLayer.include({
	...
	_saveTile: function (tile, tileUrl, existingRevision, done) {
		...
		try {
			...
			this.fire('tilecacheerror', {
				tile: tile,
				error: err
			});
			return done();
		}
		// ARC updated following comments on Github
		var doc = {
			_id: tileUrl,
			dataUrl: dataUrl,
			timestamp: Date.now()
		...
		var updatedDoc;
		if (existingRevision) {
			//newer method that wasn't working
			offlineTiles.get(tileUrl).then(function (doc) {
				//console.log('important to know if I am here');
				updatedDoc = {
					_id: tileUrl,
					_rev: doc._rev,
					dataUrl: dataUrl,
				...
				return offlineTiles.put(updatedDoc)
					.catch(function (err) {
						console.log('reverting to the older method of updating the cache');
						offlineTiles.remove(tileUrl, existingRevision);
						offlineTiles.put(doc);
					});
			}).then(function (response) {
				//will show when a tile has been updated
				//console.log('_saveTile update: ', response);
			});

		} else {
			console.log('I am not a pre-exisiting cached tile');
			offlineTiles.put(doc).then(function (doc) {
				//will show when a tile is inserted into the db
				//console.log('_saveTile insert: ', doc);
			});
		}

		//old method slightly updated
		// offlineTiles.remove(tileUrl, existingRevision);
		// }
	...
	_seedOneTile: function (tile, remaining, seedData) {
		...
		offlineTiles.get(url, function (err, data) {
			if (!data) {
